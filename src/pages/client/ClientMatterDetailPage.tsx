...
export const ClientMatterDetailPage: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const [matter, setMatter] = useState<LegalMatter | null>(null);
  const [isLoading, setIsLoading] = useState(true);

  useSEO({ title: matter ? `${matter.title} — Matter Details` : 'Matter Details', noIndex: true });
  ...
  if (isLoading) return <LoadingState message="Loading matter details..." />;
  if (!matter) return <NotFoundState title="Matter Record Not Found" returnUrl={ROUTES.CLIENT.MATTERS} />;

  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <Link to={ROUTES.CLIENT.MATTERS} className="inline-flex items-center gap-1.5 text-xs font-semibold text-slate-600 hover:text-slate-900">
